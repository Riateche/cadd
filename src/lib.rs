...
macro_rules! declare_binary_trait {
    ($trait_:ident, $trait_fn:ident, $doc:literal) => {
        ...
        pub trait $trait_<Other = Self>: Sized {
            type Output;
            fn $trait_fn(self, b: Other) -> Result<Self::Output>;
        }

        #[doc = $doc]
        pub fn $trait_fn<T1, T2>(a: T1, b: T2) -> Result<T1::Output>
        where
            T1: $trait_<T2>,
        {
            a.$trait_fn(b)
...
macro_rules! declare_unary_trait {
    ($trait_:ident, $trait_fn:ident, $doc:literal) => {
        ...
        pub trait $trait_: Sized {
            type Output;
            fn $trait_fn(self) -> Result<Self::Output>;
        }

        #[doc = $doc]
        pub fn $trait_fn<T1>(value: T1) -> Result<T1::Output>
        where
            T1: $trait_,
        {
            value.$trait_fn()
